...
class websocket_frame_generator
{
private:
    std::vector<std::uint8_t> frame_data_;

public:
    websocket_frame_generator() = default;
    ...
    websocket_frame_generator& ping(std::span<std::uint8_t const> payload = {}, bool mask = false);

    /// Create a pong frame
    /// \param payload Optional payload (max 125 bytes, usually echoes ping payload)
    /// \param mask Whether to mask the payload (default: false for server-to-client)
    /// \return Reference to this generator for method chaining
    websocket_frame_generator& pong(std::span<std::uint8_t const> payload = {}, bool mask = false);
    ...
    /// \param mask Whether to mask the payload (default: false for server-to-client)
    /// \return Reference to this generator for method chaining
    websocket_frame_generator& close(
    ...
    /// \param mask Whether to mask the payload (default: false for server-to-client)
    /// \return Reference to this generator for method chaining
    websocket_frame_generator& text(std::string_view text, bool fin = true, bool mask = false);
    ...
    /// \param mask Whether to mask the payload (default: false for server-to-client)
    /// \return Reference to this generator for method chaining
    websocket_frame_generator& binary(
            std::span<std::uint8_t const> data, bool fin = true, bool mask = false);
    ...
    /// \param mask Whether to mask the payload (default: false for server-to-client)
    /// \return Reference to this generator for method chaining
    websocket_frame_generator& continuation(
